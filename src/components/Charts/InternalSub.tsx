...
import { internalSubOptions, getColor, getRadarOrder } from "./ChartConfigs";
...
function InternalSub({
  ...
  useEffect(() => {
    ...
    dfd.readCSV(paths.internalSub).then((internalSubData) => {
      ...
      const internalSubDataInYearsRange = internalSubData.loc({
        rows: yearsFilter,
      });
      ...
      // update graph title
      internalSubOptions.plugins.title.text = `Internal Subsidies for all passenger vehicles ${minYear}-${maxYear}`;
  ...
  return (
    <div className="border rounded-lg shadow-2xl h-full flex flex-none flex-col justify-center items-center ">
      <div className="relative sm:h-full sm:w-full">
        {/* @ts-ignore */}
        <Radar data={lineData} options={internalSubOptions} />
      </div>
    </div>
  );
}
export default memo(InternalSub);
