...
import { salesOptions, getColor, getOrder } from "./ChartConfigs";
...
function Sales({
  ...
  useEffect(() => {
    try {
      dfd.readCSV(paths.sales).then((salesData) => {
        ...
        const yearsFilter = salesData["Year"]
          .ge(minYear)
          .and(salesData["Year"].le(maxYear));

        // select rows in years range
        const salesDataInYearsRange = salesData.loc({ rows: yearsFilter });

        // == Adding markets lines ==
        if (vehicleTypes.length == 0 && powertrains.length == 0) {
          const allMarkets = salesDataInYearsRange
            .groupby(["Market", "Year"])
            .col(["value"])
            .sum();
        ...
        // == Adding vehicle types lines ==
          ...
          const allTypes = salesDataInYearsRange
        ...
        // == Adding powertrains lines ==
          ...
          const allPowertrains = salesDataInYearsRange
          ...
          if (markets.length == 2 && vehicleTypes.length == 2) {
            for (const powertrain of powertrains) {
              let powertrainFilter =
                allPowertrains["Powertrain"].eq(powertrain);
              let eachPowertrain = allPowertrains.loc({
                rows: powertrainFilter,
              });
        ...
        const chartTitle =
          markets.length == 2
            ...
            : `${markets[0]} Market`;
        salesOptions.plugins.title.text = `Sales for ${chartTitle} ${minYear}-${maxYear}`;
      });
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <div>
      {/* @ts-ignore */}
      <Line data={lineData} options={salesOptions} />
    </div>
  );
}
