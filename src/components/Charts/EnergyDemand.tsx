...
import { energyDemandOptions, getColor, getOrder } from "./ChartConfigs";
...
function EnergyDemand({
  ...
  useEffect(() => {
    ...
    dfd.readCSV(paths.energyDemand).then(energyDemandData => {

      // define years range [minYear - maxYear]
      const yearsFilter = energyDemandData["Year"]
        .ge(minYear)
        .and(energyDemandData["Year"].le(maxYear));
  
      // select rows in years range
      const energyDemandDataInYearsRange = energyDemandData.loc({
        rows: yearsFilter,
      });
  
      // == Adding markets lines ==
      if (vehicleTypes.length == 0 && powertrains.length == 0) {
        const allMarkets = energyDemandDataInYearsRange
          .groupby(["Market", "Year"])
          .col(["value"])
          .sum();
      ...
      // == Adding vehicle types lines ==
        ...
        const allTypes = energyDemandDataInYearsRange
      ...
      // == Adding powertrains lines ==
        ...
        const allPowertrains = energyDemandDataInYearsRange
      ...
      energyDemandOptions.plugins.title.text = `Energy Demand for ${chartTitle} ${minYear}-${maxYear}`;
  ...
  return (
    <div>
      {/* @ts-ignore */}
      <Bar data={lineData} options={energyDemandOptions} />
    </div>
  );
}
export default memo(EnergyDemand);
